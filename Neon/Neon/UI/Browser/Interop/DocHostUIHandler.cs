...
namespace Netron.Neon
{
	...
	{
		private AxWebBrowser hostControl;
		...
		public int ShowContextMenu(int dwID, Interop.POINT pt, object pcmdtReserved, object pdispReserved) {
			...
			DWebBrowserEvents2_MenuCancelEventArgs e = new DWebBrowserEvents2_MenuCancelEventArgs(location, false);
			try {
		...
		public int TranslateUrl(int dwTranslate, string strURLIn, out string pstrURLOut) {
			DWebBrowserEvents2_TranslateUrlEventArgs e = new DWebBrowserEvents2_TranslateUrlEventArgs(strURLIn);
			try {
		...
		public void ParseDisplayName(object pbc, string pszDisplayName, int[] pchEaten, object[] ppmkOut) {
			//Trace.Fail("ParseDisplayName - " + pszDisplayName);
