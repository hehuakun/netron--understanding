...
using Netron.Neon.HtmlHelp.Storage;

namespace Netron.Neon.HtmlHelp.ChmDecoding
{
	/// <summary>
	...
	public sealed class CHMFile : IDisposable
	{
		...
		internal CHMFile(HtmlHelpSystem systemInstance, string chmFile, bool onlySystemData, DumpingInfo dmpInfo)
		{
			...
			if( File.Exists(chmFile) )
			{
				...
				Trace.WriteLine("--- Reading HtmlHelp file: " + chmFile);
				Trace.Indent();
				...
				Trace.WriteLine("--- HtmlHelp file read in " + elapsedHH.ToString());
				Trace.Unindent();

				if(_mustWriteDump)
				{
					Trace.WriteLine("--- Writing data dump ");
					Trace.Indent();
					...
					Trace.WriteLine("--- Dump written in " + elapsedD.ToString());
					Trace.Unindent();
				}
		...
		private void ReadFile(string fname, HtmlHelpFileType type)
		{
			...
			Trace.WriteLine("CHMFile.ctor() - Reading system files from " + type.ToString() + ": ");
			Trace.Indent();
			...
			if( (type != HtmlHelpFileType.CHQ) && (type != HtmlHelpFileType.CHW) )
			{
				...
				if( fileObject != null)
				{
					Trace.WriteLine("Read #SYSTEM file: ");
					Trace.Indent();
					...
					Trace.WriteLine("time elapsed - " + elapsed.ToString());
					Trace.Unindent();
				}
				...
				if( fileObject != null)
				{
					Trace.WriteLine("Read #IDXHDR file: ");
					Trace.Indent();
					DateTime dtStart = DateTime.Now;
					byte[] fileData = new byte [fileObject.Length];
					fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				{
					_dumpReadTrys++;
					Trace.WriteLine("--- Reading data dump ");
					Trace.Indent();
					DateTime dtStartD = DateTime.Now;
					...
					DateTime dtEndD = DateTime.Now;
					TimeSpan elapsedD = dtEndD-dtStartD;
					Trace.WriteLine("--- Dump read in " + elapsedD.ToString());
					Trace.Unindent();
				}
				...
				if( (!_dumpRead)||(!_dmpInfo.DumpStrings) )
				{
					...
					if( fileObject != null)
					{
						Trace.WriteLine("Read #STRINGS file: ");
						Trace.Indent();
						DateTime dtStart = DateTime.Now;
						byte[] fileData = new byte [fileObject.Length];
						fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				if( (!_dumpRead)||(!_dmpInfo.DumpUrlStr) )
				{
					...
					if( fileObject != null)
					{
						Trace.WriteLine("Read #URLSTR file: ");
						Trace.Indent();
						DateTime dtStart = DateTime.Now;
						byte[] fileData = new byte [fileObject.Length];
						fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				if( (!_dumpRead)||(!_dmpInfo.DumpUrlTbl) )
				{
					...
					if( fileObject != null)
					{
						Trace.WriteLine("Read #URLTBL file: ");
						Trace.Indent();
						DateTime dtStart = DateTime.Now;
						byte[] fileData = new byte [fileObject.Length];
						fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				if( (!_dumpRead)||(!_dmpInfo.DumpTopics) )
				{
					...
					if( fileObject != null)
					{
						Trace.WriteLine("Read #TOPICS file: ");
						Trace.Indent();
						DateTime dtStart = DateTime.Now;
						byte[] fileData = new byte [fileObject.Length];
						fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				if(!_onlySystem)
				{
					if( (!_dumpRead)||(!_dmpInfo.DumpBinaryTOC) )
					{
						...
						{
							Trace.WriteLine("Read #TOCIDX file: ");
							Trace.Indent();
							DateTime dtStart = DateTime.Now;
							byte[] fileData = new byte [fileObject.Length];
							fileObject.Read(fileData, 0, (int)fileObject.Length);
				...
				if( (_systemFile != null) && (!_onlySystem) )
				{
					if(!_systemFile.BinaryTOC)
					{
						if( (!_dumpRead)||(!_dmpInfo.DumpTextTOC) )
						{
							...
							if( fileObject != null)
							{
								Trace.WriteLine("Parsing HHC-toc: ");
								Trace.Indent();
								DateTime dtStart = DateTime.Now;
				...
				if( (_systemFile != null) && (!_onlySystem) )
				{
					if( ! _systemFile.BinaryIndex )
					{
						if( (!_dumpRead)||(!_dmpInfo.DumpTextIndex) )
						{
							...
							if( fileObject != null)
							{
								Trace.WriteLine("Parsing HHK-index: ");
								Trace.Indent();
								DateTime dtStart = DateTime.Now;
					...
					else 
					{
						if( (!_dumpRead)||(!_dmpInfo.DumpBinaryIndex) )
						{
							...
							if(storage != null)
							{
								...
								if( fileObject != null)
								{
									Trace.WriteLine("Extracting index (klinks) from BTree: ");
									Trace.Indent();
									DateTime dtStart = DateTime.Now;
							...
							if(storage != null)
							{
								...
								if( fileObject != null)
								{
									Trace.WriteLine("Extracting index (alinks) from BTree: ");
									Trace.Indent();
									DateTime dtStart = DateTime.Now;
				...
				if( (_systemFile != null) && (!_onlySystem) )
				{
					if( _systemFile.FullTextSearch)
					{
						if( (!_dumpRead)||(!_dmpInfo.DumpFullText) )
						{
							...
							if( fileObject != null)
							{
								Trace.WriteLine("Enabling fulltext-search: ");
								Trace.Indent();
								DateTime dtStart = DateTime.Now;
		...
		private bool SaveDump(DumpingInfo dmpInfo)
		{
			...
			try
			{
				Trace.WriteLine("writing dump-file header");
				...
				if(dmpInfo.DumpStrings)
				{
					...
					{
						Trace.WriteLine("writing #STRINGS");
				...
				if(dmpInfo.DumpUrlStr)
				{
					...
					{
						Trace.WriteLine("writing #URLSTR");
						writer.Write(true);
				...
				if(dmpInfo.DumpUrlTbl)
				{
					...
					{
						Trace.WriteLine("writing #URLTBL");
						writer.Write(true);
				...
				if(dmpInfo.DumpTopics)
				{
					...
					{
						Trace.WriteLine("writing #TOPICS");
						writer.Write(true);
				...
				if(dmpInfo.DumpFullText)
				{
					...
					{
						Trace.WriteLine("writing $FIftiMain");
						writer.Write(true);
				...
				{
					Trace.WriteLine("writing binary TOC");
					bWriteTOC = true;
				}

				if( (!_systemFile.BinaryTOC) && (dmpInfo.DumpTextTOC) )
				{
					Trace.WriteLine("writing text-based TOC");
					bWriteTOC = true;
				}
				...
				{
					Trace.WriteLine("writing binary index");
					bWriteIdx = true;
				}

				if( (!_systemFile.BinaryIndex) && (dmpInfo.DumpTextIndex) )
				{
					Trace.WriteLine("writing text-based index");
					bWriteIdx = true;
				}
				...
				Trace.WriteLine("writing " + _informationTypes.Count.ToString() + " information types");
				...
				Trace.WriteLine("writing " + _categories.Count.ToString() + " categories");
		...
		private bool LoadDump(DumpingInfo dmpInfo)
		{
			...
			try
			{
				...
				if(reader == null)
				{
					Trace.WriteLine("No reader returned !");
				...
				Trace.WriteLine("reading dump-file header");
				FileInfo fi = new FileInfo(_chmFileName);
				string ftime = fi.LastWriteTime.ToString("dd.MM.yyyy HH:mm:ss.ffffff");
				...
				{
					Trace.WriteLine("Unsupported dump-file format !");
					dmpInfo.SaveData(); // closes the dump
					_mustWriteDump = true;
					return false;
				}
				...
				if(ftimecheck != ftime)
				{
					Trace.WriteLine("Dump is out of date (CHM file changed during last dump creation) !");
					dmpInfo.SaveData(); // closes the dump
					_mustWriteDump = true;
				...
				if(bFlag)
				{
					...
					if(!dmpInfo.DumpStrings)
					{
						Trace.WriteLine("Dumped #STRINGS found but not expected !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(bFlagSupp)
					{
						Trace.WriteLine("reading #STRINGS");
				...
				else 
				{
					if(dmpInfo.DumpStrings)
					{
						Trace.WriteLine("Dumped #STRINGS expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					...
					if(!dmpInfo.DumpUrlStr)
					{
						Trace.WriteLine("Dumped #URLSTR found but not expected !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(bFlagSupp)
					{
						Trace.WriteLine("reading #URLSTR");
				...
				else 
				{
					if(dmpInfo.DumpUrlStr)
					{
						Trace.WriteLine("Dumped #URLSTR expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					...
					if(!dmpInfo.DumpUrlTbl)
					{
						Trace.WriteLine("Dumped #URLTBL found but not expected !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(bFlagSupp)
					{
						Trace.WriteLine("reading #URLTBL");
				...
				else 
				{
					if(dmpInfo.DumpUrlTbl)
					{
						Trace.WriteLine("Dumped #URLTBL expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					...
					if(!dmpInfo.DumpTopics)
					{
						Trace.WriteLine("Dumped #TOPICS found but not expected !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(bFlagSupp)
					{
						Trace.WriteLine("reading #TOPICS");
				...
				else 
				{
					if(dmpInfo.DumpTopics)
					{
						Trace.WriteLine("Dumped #TOPICS expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					...
					if(!dmpInfo.DumpFullText)
					{
						Trace.WriteLine("Dumped $FIftiMain found but not expected !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(bFlagSupp)
					{
						Trace.WriteLine("reading $FIftiMain");
				...
				else 
				{
					if(dmpInfo.DumpFullText)
					{
						Trace.WriteLine("Dumped $FIftiMain expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					if((_systemFile.BinaryTOC)&&(!dmpInfo.DumpBinaryTOC))
					{
						Trace.WriteLine("Binary TOC expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					} 
					...
					if((!_systemFile.BinaryTOC)&&(!dmpInfo.DumpTextTOC))
					{
						Trace.WriteLine("Text-based TOC expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(_systemFile.BinaryTOC)
						Trace.WriteLine("reading binary TOC");
					else
						Trace.WriteLine("reading text-based TOC");
				...
				else 
				{
					if((_systemFile.BinaryTOC)&&(dmpInfo.DumpBinaryTOC))
					{
						Trace.WriteLine("Binary TOC expected but no TOC dump !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					} 
					...
					if((!_systemFile.BinaryTOC)&&(dmpInfo.DumpTextTOC))
					{
						Trace.WriteLine("Text-based TOC expected but no TOC dump !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				if(bFlag)
				{
					if((_systemFile.BinaryIndex)&&(!dmpInfo.DumpBinaryIndex))
					{
						Trace.WriteLine("Binary index expected but not found !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
					...
					if(_systemFile.BinaryIndex)
						Trace.WriteLine("reading binary index");
					else
						Trace.WriteLine("reading text-based index");
				...
				else 
				{
					if((_systemFile.BinaryIndex)&&(dmpInfo.DumpBinaryIndex))
					{
						Trace.WriteLine("Binary index expected but no index in dump !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					} 
					...
					if((!_systemFile.BinaryIndex)&&(dmpInfo.DumpTextIndex))
					{
						Trace.WriteLine("Text-based index expected but no index dump !");
						dmpInfo.SaveData(); // closes the dump
						_mustWriteDump = true;
						return false; // force reload
					}
				...
				Trace.WriteLine("Reading " + nITCnt.ToString() + " information types from dump !");
				...
				Trace.WriteLine("Reading " + nITCnt.ToString() + " categories from dump !");
